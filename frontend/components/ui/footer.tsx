...
import Np_logo from "./Np_logo";
import { InstagramIcon, SpotifyIcon, TiktokIcon, YoutubeIcon } from "./socialIcons";
import Np_phrase from "./np_phrase";
        ...
        <footer className="sticky bottom-0 left-0 right-0 w-full bg-black z-50 overflow-hidden">
            <div className="left-0 right-0 max-w-full p-4 py-4">
                <div className="flex flex-col md:flex-row items-center justify-between sm:gap-0">
                    ...
                    <div className="flex flex-row items-center justify-center text-white gap-4 order-2 md:order-2">
                    ...
                    <ul className="flex flex-row items-center justify-center text-[10px] sm:text-sm text-white gap-3 order-1 md:order-3 ">
                        {dataFooter.map((data) => (
                            <li key={data.id}>
                                <Link href={data.link} className=" font-medium hover:underline me-3 md:me-5 m-auto">{data.name}</Link>
                            </li>
                        ))}
                    </ul>
                </div>
