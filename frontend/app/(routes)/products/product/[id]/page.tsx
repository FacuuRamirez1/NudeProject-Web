'use client'
import { useGetProduct } from "@/app/api/useGetProducts";
...
const ViewProductPage = ({ params }: { params: Promise<{ id: number }> }) => {
    const { id } = use(params)
    const { loading, req }: ResponseType<ProductType[]> = useGetProduct(id)
        ...
        <div className="w-full h-fit bg-[#D9D9D9] flex justify-center">
            {loading ? (
                ...
                <div className="w-full bg-white">
                    {req?.map((product: ProductType) => {
                        ...
                        const productInCart = items.some((item: ProductType) => item.id === product.id)

                        const handleAddToCart = () => {
                            if (selectedSize) {
                                addItem(product)
                            } else {
                                toast.error("Choise a size!")
                            }
                        }
                                ...
                                <div className="flex flex-row w-full md:w-[70%]">
                                    <div className="hidden md:flex flex-col m-auto justify-center w-[50%] border-r border-gray-100">
                                        <div className="relative aspect-4/5 w-full">
                                            <Image src={finalSrc(product, 0)} alt={product.productName || 'Product Image'} fill unoptimized className="object-fill" loading="eager"/>
                                        </div>
                                    </div>
                                    <div className="relative flex-1 aspect-4/5 bg-gray-50">
                                        <Image src={finalSrc(product, 1)} alt={product.productName || 'Product Image'} fill unoptimized className="object-fill" loading="eager"/>
                                    </div>
                                </div>
                                            ...
                                            <div className="grid grid-cols-5 py-4 border-y-2 justify-items-center border-[#3E2327]">
                                                ...
                                                {product.details && product.details.length > 0 ? (
                                                    product.details.map((item, index) => (
                                                        <Accordion type="multiple" defaultValue={["details"]} key={item.id ?? index} className="w-full my-2  overflow-y-auto scrollbar-hide [-ms-overflow-style:'none'] [scrollbar-width:'none'] [&::-webkit-scrollbar]:hidden">
            ...
            <div className="flex items-center justify-center min-h-screen text-center px-4">
                    <div className="flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-700">
                        <h2 className="text-7xl md:text-9xl font-black text-[#3E2723] uppercase tracking-tighter leading-none select-none pb-6">
                            Sorry!
                        </h2>
